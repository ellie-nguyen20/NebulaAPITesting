...
def test_mistral_small_3_2_24b_simple(config, api_key_scope_session):
    logger.info("🚀 Starting Mistral Small 3.2-24B text test with simple prompt")
    ...
    try:
        ...
        logger.info(f"📝 Mistral Small 3.2-24B text response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ Mistral Small 3.2-24B text test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ Mistral Small 3.2-24B text test failed: {e}")
        raise
