...
def test_qwq_32b_simple(config, api_key_scope_session):
    ...
    try:
        ...
        logger.info(f"📝 QwQ-32B text response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ QwQ-32B text test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ QwQ-32B text test failed: {e}")
        raise
