...
def test_llama_3_3_70b_simple(config, api_key_scope_session):
    ...
    try:
        ...
        logger.info(f"📝 Llama 3.3-70B text response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ Llama 3.3-70B text test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ Llama 3.3-70B text test failed: {e}")
        raise
