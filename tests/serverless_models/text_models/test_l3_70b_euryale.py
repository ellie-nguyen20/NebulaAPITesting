...
def test_l3_70b_euryale_simple(config, api_key_scope_session):
    logger.info("🚀 Starting L3-70B-Euryale text test with simple prompt")
    ...
    try:
        ...
        logger.info(f"📝 L3-70B-Euryale text response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ L3-70B-Euryale text test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ L3-70B-Euryale text test failed: {e}")
        raise
