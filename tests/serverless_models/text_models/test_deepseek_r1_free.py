...
def test_deepseek_r1_free_simple(config, api_key_scope_session):
    logger.info("🚀 Starting DeepSeek R1-Free text test with simple prompt")
    ...
    try:
        ...
        logger.info(f"📝 DeepSeek R1-Free text response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ DeepSeek R1-Free text test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ DeepSeek R1-Free text test failed: {e}")
        raise
