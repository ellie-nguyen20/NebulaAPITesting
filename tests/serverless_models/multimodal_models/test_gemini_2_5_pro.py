...
def test_gemini_2_5_pro_simple(config, api_key_scope_session):
    logger.info("🚀 Starting Gemini 2.5 Pro test with simple prompt")
    ...
    try:
        ...
        logger.info(f"📝 Gemini 2.5 Pro response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ Gemini 2.5 Pro test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ Gemini 2.5 Pro test failed: {e}")
        raise
