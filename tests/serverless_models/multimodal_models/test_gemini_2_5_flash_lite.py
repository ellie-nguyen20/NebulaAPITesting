...
def test_gemini_2_5_flash_lite_simple(config_with_api_key):
    ...
    try:
        ...
        logger.info(f"📝 Gemini 2.5 Flash Lite response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ Gemini 2.5 Flash Lite test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ Gemini 2.5 Flash Lite test failed: {e}")
        raise
