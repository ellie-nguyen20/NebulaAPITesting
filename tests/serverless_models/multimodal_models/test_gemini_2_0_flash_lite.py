...
def test_gemini_2_0_flash_lite_simple(config, api_key_scope_session):
    logger.info("🚀 Starting Gemini 2.0 Flash Lite test with simple prompt")
    ...
    try:
        ...
        logger.info(f"📝 Gemini 2.0 Flash Lite response: {content}")
        
        assert len(content) > 0, "Response content should not be empty"
        assert isinstance(content, str), "Response content should be a string"
        
        logger.info("✅ Gemini 2.0 Flash Lite test completed successfully")
        
    except Exception as e:
        logger.error(f"❌ Gemini 2.0 Flash Lite test failed: {e}")
        raise
